...
> 我们没有很好的区分进程还是线程,但是实际上应该就是进程.

#### simple_mp

两个线程,两个CPU,使用共享内存来进行交互,send和receive.
...
​	执行两次,分别指定不同的进程名称,pri用来创建共享内存,sec进程attach到这个内存中去,就利用队列进行了交互的操作.

### Symmetric Multi-process Example

研究一组进程怎么并行的处理一组相同的数据包.--->因为任务是对等的,所以叫对称的多进程.



![../_images/sym_multi_proc_app.png](https://doc.dpdk.org/guides/_images/sym_multi_proc_app.png)

上面的图片展示了两个进程处理的数据流.

从两个port上面处理:

RX0收到的包都给P0处理,RX1收到的都给P1处理.

TX是同理的.--->可以看出来这样是不会产生竞争的现象的.
